function data_filt = fmri_bandpassfilter(data, fs, f_low, f_high)
...
%
% Inputs:
%   DATA    - Time series to filter. Each column is filtered independently.
%             Accepts vectorized fMRI (T x 228453) or a single time series (T x 1).
...
%
% See also: fmri_detrend, fmri_tempsmooth, fmri_correctmovement
...
for k = 1:size(data, 2)
    ...
    N  = length(z);
    ff = (0:(N-1)) / N;
    f  = fs * (ff - round(ff));              % signed frequencies (Hz)
    ind = abs(f) < f_low | abs(f) > f_high;  % frequencies to remove
    z(ind) = 0;
    data_filt(:, k) = real(ifft(z));          % real() removes numerical noise
end
